...
const recurse = require("reftools").recurse;
...
const httpsnippetGenerator = require("./httpsnippetGenerator.js");
    ...
    if (op && op["x-code-samples"]) {
        ...
        const codeSamples = data.header.language_tabs.map((tab) => {
            ...
            const client = getLanguageClient(
                lang,
                data.options.language_clients
            );

            const sample =
                (target && samplesGenerator(target, client, data)) || "";
            return (
                (sample && generateCodeSnippet(lowerCaseLanguage, sample)) || ""
            );
        });
...
function schemaToArray(schema, offset, options, data) {
    ...
    walkSchema(schema, {}, wsState, function (schema, parent, state) {
        ...
        if (
            ...
            if (dschema.discriminator) {
                block.title +=
                    " - discriminator: " +
                    prefix +
                    dschema.discriminator.propertyName;
            }
            container.push(block);
        ...
        if (entry.type === "array" && schema.items) {
            ...
            if (schema.items.$ref) {
                // repeat for un-dereferenced schemas
                let $ref = schema.items.$ref.replace(
                    "#/components/schemas/",
                    ""
                );
                itemsType = "[" + $ref + "](#schema" + $ref.toLowerCase() + ")";
...
function getSampleInner(orig, options, samplerOptions, api) {
    ...
    if (options.sample && obj) {
        try {
            obj = JSON.parse(
                safejson(orig, undefined, undefined, options.maxDepth)
            ); // now we always limit
...
function html(markdown, header, options) {
    let preface = `<html><head><meta charset="UTF-8"><title>${md.utils.escapeHtml(
        header.title
    )}</title>`;
    if (options.respec) {
        preface +=
            '<script src="https://mermade.github.io/static/respec-widdershins.js" class="remove"></script>';
        preface += `<script class="remove">var respecConfig = ${JSON.stringify(
            options.respec
        )};</script>`;
